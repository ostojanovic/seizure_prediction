...
for idx, patient_id in enumerate(patients_epilepsiae):
    with open('*') as f:            # add path with text file with all results for the epilepsiae dataset
        file = json.load(f)
...
for idx, patient_id in enumerate(patients_ecosystem):
    with open('*') as f:            # add path with text file with all results for the epilepsyecosystem dataset
        file = json.load(f)

    accuracy_ecosystem.append(np.round(file[0]["accuracy_smote"],decimals=3))
    specificity_ecosystem.append(np.round(file[0]["specificity_smote"],decimals=3))
    sensitivity_ecosystem.append(np.round(file[0]["sensitivity_smote"],decimals=3))
    ppv_ecosystem.append(np.round(file[0]["positive_predictive_smote"],decimals=3))
    npv_ecosystem.append(np.round(file[0]["negative_predictive_smote"],decimals=3))

#################################################################### extraction finished ######################################################################

fig = plt.figure(figsize=(14,8))
gs = gridspec.GridSpec(2, 3, left=0.07, right = 0.95, wspace=0.13, hspace=0.2, top=0.9, bottom = 0.1)
...
ax4.set_xticklabels([60,70, 80, 90, 100])
ax4.set_ylim([0,4])
ax4.set_yticks([1, 2, 3])
ax4.set_yticklabels([1, 2, 3])
ax4.tick_params(axis='both', which='both', top=False, labelsize=18, length=8)

ax4.spines['right'].set_visible(False)
ax4.spines['left'].set_visible(False)
ax4.spines['top'].set_visible(False)

ax4.set_xlabel('Accuracy', fontsize=22)
...
fig.text(0.05,0.91,r"$\textbf{A}$",fontsize=26)
fig.text(0.405,0.91,r"$\textbf{B}$",fontsize=26)
fig.text(0.71,0.91,r"$\textbf{C}$",fontsize=26)
fig.text(0.05,0.47,r"$\textbf{D}$",fontsize=26)
fig.text(0.405,0.47,r"$\textbf{E}$",fontsize=26)
fig.text(0.71,0.47,r"$\textbf{F}$",fontsize=26)
