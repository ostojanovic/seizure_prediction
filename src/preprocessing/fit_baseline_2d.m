function [W_baseline,H_baseline,W_model_baseline,H_model_baseline,W_parameters_baseline,H_parameters_baseline,Models_baseline] = fit_baseline_2d(path_directory,baseline_directory,files,num_files,num_channels,num_windows,sample,run_nr)
...
W_baseline = zeros(num_channels, num_windows, nnmf_order);
H_baseline = zeros(num_channels, num_freq_bins, nnmf_order);

W_model_baseline = zeros(num_channels, num_windows, nnmf_order);
H_model_baseline = zeros(num_channels, num_freq_bins, nnmf_order);

W_parameters_baseline = zeros(num_channels, time_order+1, nnmf_order);
H_parameters_baseline = zeros(num_channels, freq_order+3, nnmf_order);

Models_baseline = zeros(num_channels, num_windows, num_freq_bins, nnmf_order+nnmf_order);
...
for i = 1:num_files      
    ...
    for IDXC = 1:num_channels
        ...
        Models_baseline(IDXC,:,:,:) = permute(Models_Baseline,[2,3,1]);
    ...
    savename_baseline = strcat(path_directory,baseline_directory,'2dim/',run_nr,'/',sample,'/Model_',savename_part{2});
