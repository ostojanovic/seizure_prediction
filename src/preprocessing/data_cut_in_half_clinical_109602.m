...
base_directory = 'patient_109602_extracted_seizures/data_clinical_109602';

load(strcat(path,base_directory,'/Data_clinical_9.mat'))   % change the name down there!!!
...
figure()
plot(temp(round(pstart):round(pend),57:68) + repmat(56:67,round(pend-pstart+1),1)*8000)
title('channels 57 to 68')

figure()
plot(temp(round(pstart):round(pend),69:75) + repmat(69:75,round(pend-pstart+1),1)*8000)
title('channels 69 to 75')
...
savename_1 = strcat(path,base_directory,'/Data_preictal_9.mat');
savename_2 = strcat(path,base_directory,'/Data_ictal_9.mat');
